...
sprites.onOverlap(SpriteKind.Player, SpriteKind.Enemy, function (jugador, enemigo) {
    if (invulnerable) {
        return
    }
    invulnerable = true
    vida_jugador.value -= 10
nena.startEffect(effects.fountain, 500)
    music.play(music.createSoundEffect(WaveShape.Noise, 2000, 1, 255, 0, 120, SoundExpressionEffect.None, InterpolationCurve.Curve), music.PlaybackMode.InBackground)
    pause(600)
    invulnerable = false
})
...
function crear_elon () {
    elon = sprites.create(assets.image`elon-front`, SpriteKind.Enemy)
    tiles.placeOnRandomTile(elon, assets.tile`transparency16`)
    elon.vx = 0
    elon.vy = 0
    elon.ax = 0
    elon.ay = 0
    elon.setFlag(SpriteFlag.Ghost, true)
}
sprites.onOverlap(SpriteKind.Projectile, SpriteKind.Enemy, function (bala, enemigo3) {
    let i: number;
sprites.destroy(bala)
    if (enemigos.indexOf(enemigo3) >= 0) {
        i = enemigos.indexOf(enemigo3)
        barras_enemigo[i].value += -10
        if (barras_enemigo[i].value <= 0) {
            sprites.destroy(enemigo3)
            if (nivel == 2 && !(en_transicion)) {
                muertes_n2 += 1
...
tiles.setCurrentTilemap(tilemap`nivel0`)
    ...
    for (let index = 0; index < 3; index++) {
        s = sprites.create(assets.image`rose`, SpriteKind.Rose)
        tiles.placeOnRandomTile(s, assets.tile`miMosaico`)
    }
}
...
statusbars.onZero(StatusBarKind.Health, function (vida) {
    music.stopAllSounds()
    game.over(false)
